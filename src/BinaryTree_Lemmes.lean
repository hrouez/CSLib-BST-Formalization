...
/-- compare x v = .lt si et seulement si x < v -/
lemma compare_eq_lt (x v : α) :
    compare x v = .lt ↔ x < v :=
  compare_lt_iff_lt

/-- compare x v = .gt si et seulement si v < x -/
lemma compare_eq_gt (x v : α) :
    compare x v = .gt ↔ v < x :=
  compare_gt_iff_gt

/-- compare x v = .eq si et seulement si x = v -/
lemma compare_eq_eq (x v : α) :
    compare x v = .eq ↔ x = v :=
  compare_eq_iff_eq
...
lemma IsBST_node_iff (l r : BinaryTree α) (v : α) :
    IsBST (.node l v r) ↔
    IsBST l ∧ IsBST r ∧
    (∀ x ∈ l.toList, x < v) ∧
    (∀ x ∈ r.toList, v < x) :=
